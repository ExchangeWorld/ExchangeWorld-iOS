...
class ObjectInfoFromProfileViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        case "byCategory":
            ...
            ObjImageView.sd_setImage(with: URL(string: imageURLtoImage(input: self.object.photo_path)), placeholderImage: UIImage(named: "loading"), options: [.continueInBackground, .progressiveDownload])
            ObjCategoryIcon.image = UIImage(named: self.object.category)
            ObjNameLabel.text = self.object.name
            ObjOwnerImage.sd_setImage(with: URL(string: self.object.owner.photo_path), placeholderImage: UIImage(named: "loading"), options: [.continueInBackground, .progressiveDownload])
            ObjOwnerName.text = self.object.owner.name
    ...
    func imageURLtoImage(input :String) -> String {
        let rawPath = input
